...
function sb_db_initiate($db_server, $db_name, $db_user, $db_passwd, $db_prefix, $db_options = array())
{
	...
	if (!isset($smcFunc['db_fetch_assoc']))
		$smcFunc += array(
			...
			'db_server_info'            => 'sb_db_get_server_info',
 ...
 */
function sb_db_select($database, $connection = null)
{
 ...
 */
function sb_db_get_server_info($connection = null)
{
	global $db_connection;
...
function sb_db_query($identifier, $db_string, $db_values = array(), $connection = null)
{
	...
	if ($ret === false && empty($db_values['db_error_skip']))
		$ret = sb_db_error($db_string, $connection);
 ...
 */
function sb_db_affected_rows($connection = null)
{
	global $db_connection;
...
function sb_db_insert($method, $table, $columns, $data, $keys, $returnmode = 0, $connection = null)
{
	...
	if ($with_returning)
	{
		if ($returnmode == 1 && empty($return_var))
			$return_var = sb_db_insert_id($table, $keys[0]) + count($insertRows) - 1;
		elseif ($returnmode == 2 && empty($return_var))
		{
			$return_var = array();
			$count = count($insertRows);
			$start = sb_db_insert_id($table, $keys[0]);
			for ($i = 0; $i < $count; $i++)
				$return_var[] = $start + $i;
		}
		return $return_var;
