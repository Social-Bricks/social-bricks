		...
		elseif ($xml_format == 'atom')
			$data[] = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'published',
						'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['date_registered']),
					),
					array(
						'tag' => 'updated',
						'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['last_login']),
					),
					array(
						'tag' => 'id',
						'content' => $guid,
		...
		// More logical format for the data, but harder to apply.
			...
			$data[] = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'time',
						'attributes' => array('label' => $txt['date_registered'], 'UTC' => sb_gmstrftime('%F %T', $row['date_registered'])),
		...
		elseif ($xml_format == 'atom')
		{
			...
			$data[] = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'published',
						'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['poster_time']),
					),
					array(
						'tag' => 'updated',
						'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', empty($row['modified_time']) ? $row['poster_time'] : $row['modified_time']),
					),
					array(
						'tag' => 'id',
						'content' => $guid,
		...
		else
		{
			...
			$data[] = array(
				...
				'content' => array(
					array(
						'tag' => 'time',
						'attributes' => array('label' => $txt['date'], 'UTC' => sb_gmstrftime('%F %T', $row['poster_time'])),
...
function getXmlProfile($xml_format)
{
	...
	elseif ($xml_format == 'atom')
	{
		$data[] = array(
			...
			'content' => array(
				...
				array(
					'tag' => 'published',
					'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $profile['registered_timestamp']),
				),
				array(
					'tag' => 'updated',
					'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $profile['last_login_timestamp']),
				),
				array(
					'tag' => 'id',
					'content' => $guid,
	...
	else
	{
		...
		$data = array(
			...
			array(
				'tag' => 'last-login',
				'attributes' => array('label' => $txt['lastLoggedIn'], 'UTC' => sb_gmstrftime('%F %T', $profile['last_login_timestamp'])),
				'content' => timeformat($profile['last_login_timestamp'], false, 'forum'),
			),
			array(
				'tag' => 'registered',
				'attributes' => array('label' => $txt['date_registered'], 'UTC' => sb_gmstrftime('%F %T', $profile['registered_timestamp'])),
		...
		else
		{
			...
			$data[] = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'modified_time',
						'attributes' => !empty($row['modified_time']) ? array('label' => $txt['modified_time'], 'UTC' => sb_gmstrftime('%F %T', $row['modified_time'])) : null,
...
function getXmlPMs($xml_format, $ascending = false)
{
	...
	$select_id_members_to = 'GROUP_CONCAT(pmr.id_member)';

	$select_to_names = 'GROUP_CONCAT(COALESCE(mem.real_name, mem.member_name))';
		...
		elseif ($xml_format == 'atom')
		{
			$item = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'updated',
						'content' => sb_gmstrftime('%Y-%m-%dT%H:%M:%SZ', $row['msgtime']),
					),
					array(
						'tag' => 'title',
						'content' => $row['subject'],
		...
		else
		{
			...
			$item = array(
				...
				'content' => array(
					...
					array(
						'tag' => 'sent_date',
						'attributes' => array('label' => $txt['date'], 'UTC' => sb_gmstrftime('%F %T', $row['msgtime'])),
