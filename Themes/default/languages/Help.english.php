...
$helptxt['ban_members'] = '<strong>Ban Members</strong><br>
	Social Bricks provides the ability to &quot;ban&quot; users, which can be a useful course of action against users that have
...
$helptxt['registrations'] = '<strong>Registration Management</strong><br>
	...
	<ul class="normallist">
		...
		<li>
			...
			You can add or remove anything from the default registration agreement, which is included in Social Bricks.<br><br>
		</li>
		<li>
...
$helptxt['manage_files'] = '
	<ul class="normallist">
		<li>
			...
			Browse through all the attachments, avatars and thumbnails stored by Social Bricks.<br><br>
		</li><li>
...
$helptxt['cache_enable'] = 'Social Bricks performs caching at a variety of levels. The higher the level of caching enabled the more CPU time will be spent retrieving cached information. If caching is available on your machine it is recommended that you try caching at level 1 first.';
$helptxt['cache_memcached'] = 'If you are using memcached you need to provide the server details. This should be entered as a comma separated list as shown in the example below:<br><br>	&quot;server1,server2,server3:port,server4&quot;<br><br>Note that if no port is specified Social Bricks will use port 11211 unless the host contains a slash, then it is assumed to be an alternative transport and the port will be set to 0. Social Bricks will attempt to perform rough/random load balancing across the specified servers.';
...
$helptxt['timeLoadPageEnable'] = 'This will show the time in seconds Social Bricks took to create the page at the bottom of the board.';
...
$helptxt['attachment_image_paranoid'] = 'Selecting this setting will enable very strict security checks on image attachments. <strong>Warning!</strong> These extensive checks can fail on valid images too. It is strongly recommended to only use this setting together with image re-encoding, in order to have Social Bricks try to resample the images which fail the security checks: if successful, they will be sanitized and uploaded. Otherwise, if image re-encoding is not enabled, all attachments failing checks will be rejected.';
...
$helptxt['automanage_attachments'] = 'By default, Social Bricks puts new attachments into a single folder. For most sites this is not a problem, but as a site grows it can be useful to have multiple folders to store attachments in.<br><br>This setting allows you to set whether you manage these folders yourself (e.g. creating a second folder and moving to it when you are ready) or whether you let Social Bricks do it, based on criteria, such as when the current directory reaches a given size, or breaking down folders by years or even months on very busy sites.';
...
$helptxt['avatar_paranoid'] = 'Selecting this setting will enable very strict security checks on avatars. <strong>Warning!</strong> These extensive checks can fail on valid images too. It is strongly recommended to only use this setting together with avatar re-encoding, in order to have Social Bricks try to resample the images which fail the security checks: if successful, they will be sanitized and uploaded. Otherwise, if re-encoding of avatars is not enabled, all avatars failing checks will be rejected.';
...
$helptxt['localCookies'] = 'Social Bricks uses cookies to store login information on the client computer.
...
$helptxt['legacy_bbc'] = 'Legacy BBCodes are obsolete and cannot be used in new posts. However, they will be parsed in existing posts if enabled here.<br><br>Enabling legacy BBCodes is only useful if your forum was upgraded from a previous version of Social Bricks.<br><br>Also, even when they are enabled, some legacy BBCodes will not be rendered in the same way that they once were. For example, the [flash] BBCode will simply show a link to the Flash content instead of embedding it.';
...
$helptxt['xmlnews_attachments'] = 'Allows links to attachments to be enclosed within your XML/RSS feeds. Turn this on if you want to use your forum as a podcasting, broadcatching, or similar content distribution platform.
	...
	<br><br>
	Some feed formats only allow one enclosed attachment per post. If a post has multiple attachments, Social Bricks will enclose the largest one in these feeds.';
...
$helptxt['enable_password_conversion'] = 'By enabling this setting, Social Bricks will attempt to detect passwords stored in other formats and convert them to the format Social Bricks uses. Typically this is used for forums converted to Social Bricks, but may have other uses as well. Disabling this prevents a user from logging in using their password after a conversion and they would need to reset their password.';
...
$helptxt['secret_why_blank'] = 'For your security, your password and the answer to your secret question are encrypted so that the Social Bricks software will never tell you, or anyone else, what they are.';
...
$helptxt['membergroups'] = 'In Social Bricks there are two types of groups that your members can be part of. These are:
	<ul class="normallist">
...
$helptxt['whytwoip'] = 'Social Bricks uses various methods to detect user IP addresses. Usually these two methods result in the same address but in some cases more than one address may be detected. In this case Social Bricks logs the two most recent addresses, and uses them both for ban checks (etc). You can click on either address to track that IP and ban if necessary.';
...
$helptxt['image_proxy_maxsize'] = 'Maximum image size that the SSL image proxy will cache: bigger images will be not be cached. Cached images are stored in your Social Bricks cache folder, so make sure you have enough free space.';
...
$helptxt['cron_is_real_cron'] = 'By default, Social Bricks adds to every page a small bit of JavaScript that prompts the user\'s browser to trigger the cron.php file in the background. This approach allows Social Bricks to execute its scheduled tasks on a regular basis without requiring the admin to do any extra server configuration. However, some admins might prefer to configure their servers to directly execute cron.php on a regular basis. If you have done this, you can disable the JavaScript method.<br><br><strong>Note:</strong> Configuring your server to run scheduled tasks this way cannot be done from within Social Bricks. You will need to use your server configuration software to do so. Typical tools for this purpose include <code>cron</code> (for Linux servers), <code>schtasks.exe</code> (for Windows servers), and <code>launchd</code> (for macOS servers). Most web hosting companies provide some sort of control panel interface to allow their customers to access these tools.<br><br>The path and URL for your forum\'s cron.php are:<br>Path: %1$s/cron.php<br>URL: %2$s/cron.php';

$helptxt['languages_txt'] = 'These language entries are used throughout Social Bricks to define localized versions of text for different languages.<br><br>If you see a value like <code>&#37;1$s</code> in an entry, it is a placeholder that gets filled in with a real value at runtime. Make sure to keep such placeholders in your customized text.';
